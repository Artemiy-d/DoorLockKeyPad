...
typedef enum
{
  ...
  USER_ID_MESSAGING = 0x03,
  USER_ID_PASSAGE_MODE = 0x04,
...
typedef enum
{
  KEYPAD_MODE_NORMAL = 0x00,
  KEYPAD_MODE_VACATION = 0x01,
  KEYPAD_MODE_PRIVACY = 0x02,
  KEYPAD_MODE_LOCK_OUT = 0x03
}
keypad_mode_status_t;

typedef keypad_mode_status_t KEYPAD_MODE;
...
typedef struct SUserCodeInfo
{
  uint8_t keyPadMode;
  uint8_t masterCode[USERCODE_MAX_LEN];
  uint8_t masterCodeLength;
} SUserCodeInfo;
...
typedef struct s_CC_userCodeInfo_data_t_
{
  RECEIVE_OPTIONS_TYPE_EX rxOptions; /**< rxOptions */
  uint8_t userIdentifier;
} s_CC_userCodeInfo_data_t;
 ...
 */
extern uint16_t CC_UserCode_UsersNumberReport_handler( );

extern KEYPAD_MODE CC_UserCode_KeypadModeReport_handler( );

extern e_cmd_handler_return_code_t CC_UserCode_KeypadModeSet_handler( KEYPAD_MODE mode );

extern void CC_UserCode_MasterCodeReport_handler( uint8_t* code, uint8_t* length );

extern e_cmd_handler_return_code_t CC_UserCode_MasterCodeSet_handler( const uint8_t* code, uint8_t length );
...
extern bool CC_UserCode_ExtendedReport_handler(
    uint16_t id,
...
extern uint16_t CC_UserCode_ChecksumReport_handler();

extern bool isUserCodeValid(const uint8_t* code, uint8_t length, bool allowEmptyCode);
