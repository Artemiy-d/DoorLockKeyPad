...
AssociationInit(bool forceClearMem, void * pFS)
{
  ...
  if ((ECODE_NVM3_OK != errCode) || (ZAF_FILE_SIZE_ASSOCIATIONINFO != dataLen) || (true == forceClearMem))
  {
    ...
    for (uint8_t endpoint = 0; endpoint < (NUMBER_OF_ENDPOINTS + 1); endpoint++)
    {
      ...
      {
        for (node_id_t node = 0; node < MAX_ASSOCIATION_IN_GROUP; node++)
        {
          tAssociationInfo.Groups[endpoint][group].subGrp[node].node.nodeId = FREE_VALUE; // Free
        }
      }
    }
  ...
  else
  {
    ...
    nvm3_readData(pFileSystem, ZAF_FILE_ID_ASSOCIATIONINFO, &tAssociationInfo, sizeof(SAssociationInfo));
    ...
    for (uint8_t endpoint = 0; endpoint < (NUMBER_OF_ENDPOINTS + 1); endpoint++)
    {
      for (uint8_t group = 0; group < MAX_ASSOCIATION_GROUPS; group++)
      {
        ...
        {
          if(0 == tAssociationInfo.Groups[endpoint][group].subGrp[node].node.nodeId)
          {
            tAssociationInfo.Groups[endpoint][group].subGrp[node].node.nodeId = FREE_VALUE; // Free
            writeFile = true;
          }
